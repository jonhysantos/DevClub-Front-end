    ...
    arrayProduct.forEach(product => {
        myLi += `
            <li>
                <img src=${product.src}>
                <p>${product.name}</p>
                <p class="item-class">${currencyFormat(product.price)}<p>
            </li>
        `
    })
    list.innerHTML = myLi
...
function sumAll(){
    ...
    list.innerHTML =  `<li>O total de todos os games ${currencyFormat(totalValue)}</li>`
}
...
btnFilterAll.addEventListener('click',filterHorror);